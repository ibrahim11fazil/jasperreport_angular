...
public interface MawaredRepository extends JpaRepository<MawaredMaster, Long> {
    ...
    @Query(value = " select a.current_grade_date,a.next_grade_date,a.emp_no from tac_jobcard_grade a where emp_no in (select empno from user_sap_ws_mini a " +
                     " where a.run_date=(select max(run_date) from user_sap_ws_mini where legacycode=:jobId) " +
                     " and legacycode=:jobId) ", nativeQuery = true)

    List<Object[]> LegacyCodeWithGradeDate(@Param("jobId") String jobId);
   ...
   // List<MawaredMaster> findByNameIn(@Param("name_in") String name);
    ...
    @Query(value = "SELECT DISTINCT JOB, Case when case when JOB_DESC_AR is null then JOB_DESC else JOB_DESC_AR END is null then JOB else  case when JOB_DESC_AR is null then JOB_DESC else JOB_DESC_AR END end AS JOB_DESC_AR ,Case when case when JOB_DESC is null then JOB_DESC_AR else JOB_DESC END is null then JOB else  case when JOB_DESC is null then JOB_DESC_AR else JOB_DESC END end AS JOB_DESC FROM USER_SAP_WS_MINI ORDER BY JOB_DESC_AR ", nativeQuery = true)
